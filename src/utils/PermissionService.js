...
class PermissionService {
    ...
    try {
      ...
      if (Platform.Version >= 33) {
        ...
        relevantPermissions = permissions.filter(p => 
          p.includes('READ_MEDIA_') || p.includes('READ_MEDIA_')
        );
      ...
      const results = await Promise.all(
        relevantPermissions.map(permission => PermissionsAndroid.check(permission))
      );
  ...
  static async hasCameraPermissions() {
    try {
      ...
      const permissionsGranted = await AsyncStorage.getItem(CAMERA_PERMISSION_GRANTED_KEY);
  ...
  static async hasStoragePermissions() {
    try {
      ...
      const permissionsGranted = await AsyncStorage.getItem(PERMISSION_GRANTED_KEY);
      if (permissionsGranted === 'true') {
